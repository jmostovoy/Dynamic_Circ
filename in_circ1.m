...
%% Fixing CV_now function Part I

tc1=rem(0,T); %tc=time in the current cycle, 
             %measured from start of systole.
if(tc1<TS)
  e1=(1-exp(-tc1/tauS))/(1-exp(-TS/tauS));
  C(iLV)=CLVD*(CLVS/CLVD)^e1;
else
  e1=(1-exp(-(tc1-TS)/tauD))/(1-exp(-(T-TS)/tauD));
  C(iLV)=CLVS*(CLVD/CLVS)^e1;
end
C(isa)=Csa;
C(isv)=Csv;
%% Fixing CV_now function Part II
tc2=rem(0,T); %tc=time in the current cycle, 
             %measured from start of systole.
if(tc2<TS)
  e2=(1-exp(-tc2/tauS))/(1-exp(-TS/tauS));
  C(iRV)=CRVD*(CRVS/CRVD)^e2;
else
  e2=(1-exp(-(tc-TS)/tauD))/(1-exp(-(T-TS)/tauD));
  C(iRV)=CRVS*(CRVD/CRVS)^e2;
end
C(ipa)=Cpa;
C(ipv)=Cpv;
%% Back to normal
